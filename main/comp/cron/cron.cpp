...
cron_job *cron_job_create(const char *schedule, cron_job_callback callback, void *data)
{
  ...
  cron_job *job = (cron_job *)calloc(sizeof(cron_job), 1);
  if (job == NULL)
    goto end;
...
void cron_schedule_task(void *args)
{
  ...
  if (args != NULL)
  {
    if (strncmp((char*)args, "R1", 2) == 0) // OK I ADMIT IT, ITS NOT THE MOST BEAUTIFUL CODE EVER, BUT I NEED IT TO BE TESTABLE... DON'T WANT TO GROW OLD WAITING FOR TIME TO PASS... :P
      r1 = 1;
  }
