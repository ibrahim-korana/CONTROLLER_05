...
void *tftp_file_open(const char *fname, const char *mode, int is_write)
{
    ...
    if (!strcmp(mode, "octet")) {
        if (is_write) {
           ...
           fd = fopen(fname_local,"w+");
        } else {
           // fd = open(fname_local, O_RDONLY, 0);
           fd = fopen(fname_local,"r");
        }
    } else {
...
{
    FILE *fd = (FILE *)handle;   
    printf("file write pos=%d len=%d\n",pos,len);
    fseek(fd, pos, SEEK_SET);
    uint16_t kk = fwrite(buff,len,1,fd);
    fflush(fd);
    return kk;
}
...
int tftp_file_read(void *handle, int pos, void *buff, int len)
{
    ...
    printf("file read %d\n",len);
    fseek(fd, pos, SEEK_SET);
    uint16_t kk = fread(buff,len,1,fd);
    return kk;
}
...
static int tftp_main(int argc, char **argv)
{
    ...
    printf("TFTP server start ediliyor\n");
...
void register_tftp(void)
{
    ...
    server = tftp_server_create("/config", 69);
    tftp_server_write_set(server, 1);
    xTaskCreate(tftp_server_thread, "tftps", 4096, server, 5, NULL); 
